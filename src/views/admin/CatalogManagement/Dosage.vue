  ...
  <ms-popup v-if="showPopup" @close="closePopup" @save="save">
    ...
    <template v-slot:body>
      <div class="field-item">
        <div style="width: 150px;">Tên liều dùng</div>
        <ms-input
          :inpPopupDetail="true"
          v-model="currentItem.dosageName"
        ></ms-input>
      </div>
    </template>
...
export default {
  ...
  data() {
    return {
      title: "Danh sách liều dùng",
      columns: [
        { name: "Tên liều dùng", field: "dosageName", width: "200px" }, // Tên cột "Tên" với độ rộng 200px
      ],
      data: [],
      total: 0,
      payload: {
      ...
      showPopup: false,
      titleDetail: "Chi tiết liều dùng",
      currentItem: {},
      isAdd: false,
      idField: "dosageID",
      dataExamCate: [],
  ...
  methods: {
    ...
    async loadData(param = {}) {
      ...
      let res = await dosageAPI.getListAsync(payload);
    ...
    async addoredit(data = {}, isNew = false) {
      ...
      me.isAdd = isNew;
      me.currentItem = data;
      let resExamCate = await dosageAPI.getDataComboboxAsync();
    ...
    async save() {
      const me = this;
      let res = await dosageAPI.insertOrUpdateAsync(me.currentItem);
      if (me.isAdd) {
        res
    ...
    async deleteRow(data) {
      const me = this;
      let res = await dosageAPI.deleteAsync(
